...
public final class SWFUpload extends JavaScriptObject {
  ...
  /**
   * Cause the Flash Control to display a File Selection Dialog window. A single
   * file may be selected from the Dialog window.
   * <p>
  ...
  /**
   * Cause the Flash Control to display a File Selection Dialog window. Multiple
   * files may be selected from the Dialog window
   * <p>
   * Calling selectFile begins the File Event Chain.
   * 
   * @deprecated Not compatible with Flash Player 10.
  ...
  // The Stats Object may be modified. This is useful if you wish to change the
   ...
   * @return a File Object or null if not found, or null if the index is out of
   *         range
   */
   ...
   *          whether SWFUpload sends post parameters on the query string rather
   *          than in the post
   */
  ...
  /**
   * Change the image used in the Flash Button. The image url may be relative to
   * the Module (use GWT.getModuleBaseURL()) or an absolute path or fully
   * qualified URL.
   * <p>
   * Any image format supported by Flash can be loaded. The most notable formats
   * are <tt>JPG</tt>, <tt>GIF</tt>, and <tt>PNG</tt>.
   * <p>
   * <p>
   * The image is expected to contain four variations combined into one file,
   * with each one stacked vertically below the other. Each version represents a
   * different button state. The order of the states are: normal, hover, click,
   * disabled.
   * <p>
   * The total dimensions of the image file should be 1x the width of the button
   * and 4x the height.
   * 
   * @param url
  ...
  /**
   * Sets the text that should be displayed over the Flash button. Text that is
   * too large and overflows the button size will be clipped.
   * <p>
  ...
  /**
   * Sets the action taken when the Flash button is clicked. Valid action values
   * are taken from the {@link ButtonAction} constants.
   */
